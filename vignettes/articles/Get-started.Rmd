...
The function `r configure_environment()` can also be used interactively to manually configure an existing environment, or to create one from scratch.
...
- `have_python_deps(envname, py_pkgs)` to check if a `Python` packages list is installed in `envname`.
- `load_conda_env(envname)` to load a specific environment.
